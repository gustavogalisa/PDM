...
class MainActivity : AppCompatActivity() {
    ...
    inner class OnItemLongClick: AdapterView.OnItemLongClickListener{
        override fun onItemLongClick(
            ...
            rmDialog(this@MainActivity, position, desejo)
            return true
        }
    }

    fun rmDialog(activity: Activity, position: Int, desejo: Desejo) {
        AlertDialog.Builder(activity).apply {
            ...
            setMessage("Você quer excluir o item " + desejo.toString() + "?")
            ...
            setNegativeButton("Cancelar") { _, _ ->
                Toast.makeText(this@MainActivity, "Exclusão cancelada", Toast.LENGTH_SHORT).show()
            }
        }.create().show()
    }
}